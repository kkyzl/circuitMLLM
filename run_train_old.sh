#!/bin/bash
#SBATCH --job-name=analogseeker_sft      # 工作名稱
#SBATCH --partition=dev              # 使用的 partition (請根據你的系統修改)
#SBATCH --time=2:00:00                # 執行時間上限 (小時:分鐘:秒)
#SBATCH --account=MST111121
#SBATCH --nodes=1                     # (-N) Maximum number of nodes to be allocated
#SBATCH --gpus-per-node=4             # Gpus per node
#SBATCH --cpus-per-task=16             # (-c) Number of cores per MPI task
#SBATCH --ntasks-per-node=1
#SBATCH --ntasks=1
#SBATCH -o analogseeker_sft_%j.log        # output file (%j expands to jobId)
#SBATCH -e analogseeker_sft_%j.err        # output file (%j expands to jobId)
...
OUTPUT_DIR=./output
NGPU=8
MODE=legacy   # "deepspeed" or "legacy"
